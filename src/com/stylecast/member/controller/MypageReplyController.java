...
import com.stylecast.daily.model.service.ReplyService;
...
public class MypageReplyController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(session.getAttribute("loginUser") == null) { // 로그인 전
			...
			// 페이지바용 변수 값 담기
			listCount = new ReplyService().selectMyReplyListCount(memNo);
			...
			// 댓글 전체 조회
			ArrayList<Reply> list = new ReplyService().selectMyReplyList(memNo, pi);
			
			request.setAttribute("pi", pi);
			request.setAttribute("list", list);
			
			request.getRequestDispatcher("views/mypage/mypageReply.jsp").forward(request, response);
		}	
	}
