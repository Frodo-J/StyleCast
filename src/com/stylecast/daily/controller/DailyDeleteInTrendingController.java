...
/**
 * Servlet implementation class DailyDeleteInTrendingController
 */
@WebServlet("/deleteDaily.th")
public class DailyDeleteInTrendingController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public DailyDeleteInTrendingController() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(r != null) {
			...
			response.sendRedirect(request.getContextPath() + "/detail.th?tno="+ tno + "&dno=" + dno);			
		}else {
			int result = new DailyService().deleteDaily(dno);
			if(result > 0) {
				request.getSession().setAttribute("alertMsg", "게시글이 성공적으로 삭제되었습니다.");
				response.sendRedirect(request.getContextPath() + "/list.to?tpage=" + tno);			
			}else {
